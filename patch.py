...
if len(svn_projects) > 0:
    ...
    alps_dir = raw_input(yellow + 'Not found svn project, Please input path: ' + default)
...
if len(patches) > 0:
    ...
    for p in patches:
        print('    ' + p)
    if 'yes' != raw_input(
                            yellow + 'Are you sure the patches\'s sequence is right, it is important (yes/no): ' + default):
        exit()
else:
